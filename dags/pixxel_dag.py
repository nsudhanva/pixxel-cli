...
def calculate_index():
    ...
    upload_processed_file_to_gcs('pixxel', 'output_' + tile_key + '.tif', 'output_' + tile_key + '_airflow.tif')
...
def check_s3_modified():
    ...
    with open('modifications_previous.json', "r", encoding='utf-8') as mod:
        ...
        previous_modified_date = datetime.datetime.strptime(data['LastModified'][:10], '%Y-%m-%d')
        new_modified_date = datetime.datetime.strptime(str(response['Contents'][0]['LastModified'])[:10], '%Y-%m-%d')
