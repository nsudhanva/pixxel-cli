...
def request_image_files_from_s3(files):
    ...
    for i in files:
        tile_key = 'sentinel-s2-l2a/tiles/43/P/FP/2020/5/6/0/R60m/' + i
        print(tile_key)
...
def calculate_index():
    ...
    files = ['B8A.jp2', 'B11.jp2']

    tile_key = 'sentinel-s2-l2a/tiles/43/P/FP/2020/5/6/0/R60m/'
    ...
    with rasterio.open('output_' + 'ndmi' + '_' + tile_key + '.tif', 'w', **profile) as dataset:
        dataset.write(normalized_difference.astype(rasterio.float32))

    upload_processed_file_to_gcs('pixxel', 'output_' + 'ndmi' + '_' + tile_key + '.tif', date_path + '/output_'  + 'ndmi' + '_' + tile_key + '_airflow.tif')
